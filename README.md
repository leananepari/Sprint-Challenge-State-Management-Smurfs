...
- [x] What problem does the context API help solve?<br>
Context API makes data available to all components instead of passing it down as props parent to child, which is also referred as 'prop drilling'.
- [x] In your own words, describe `actions`, `reducers` and the `store` and their role in Redux. What does each piece do? Why is the store known as a 'single source of truth' in a redux application?<br>
Store - is a single object where all of our applications state lives, hence a single source of truth.<br>
Actions - are used to discribe actions needed to be performed for your application.<br>
Reducers - are function that update our state based on actions being performed. 
- [x] What is the difference between Application state and Component state? When would be a good time to use one over the other?<br>
Application state - is a global state meaning all components can access it.<br>
Component state - is a local state to that component and can be updated within that component and passed down to children components if needed. A good use case would be a form component.
- [x] Describe `redux-thunk`, what does it allow us to do? How does it change our `action-creators`?<br>
Redux thunk is a middleware that lets us call action creators that return a function instead of an object which is useful for async operations.
- [x] What is your favorite state management system you've learned and this sprint? Please explain why!<br>
I liked using all of them. Redux is great for large data management but for a quick start react usereducers hook or context api would be great.
